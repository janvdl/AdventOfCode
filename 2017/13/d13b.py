...
# change this to a function to return severity
def is_safe(start_delay):
    packet_pos = 0
    for picosecond in range(start_delay, last_scanner_index + start_delay + 1):
...
start_delay = 0
while not is_safe(start_delay):
    start_delay += 1
print(f"Delay start by {start_delay} picoseconds to pass safely")