...
i = -1
lines = [line.strip() for line in open('input_a.txt', 'r').readlines()]
    ...
    if len(nodes[node]) > 1: # no point looking for antinodes if there's only one node
        for i in range(len(nodes[node])):
            for j in range(i + 1, len(nodes[node])):
                ...
                antinode1 = [node1[0] - x_dist, node1[1] - y_dist if node1_left else node1[1] + y_dist]
                antinode2 = [node2[0] + x_dist, node2[1] + y_dist if node1_left else node2[1] - y_dist]
